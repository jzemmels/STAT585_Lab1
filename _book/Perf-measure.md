...
#> 1 sqrt(x)    485.92ns 591.1ns 531.09ns 14.1µs  1691716.      848B     0
#> 2 x^0.5        8.39µs  10.5µs   9.47µs 59.9µs    94900.      848B     0
...
#> 1 sqrt(x)    485.92ns 531.09ns  1691716.     0
#> 2 x^0.5        8.39µs   9.47µs    94900.     0
```
...
As with all microbenchmarks, pay careful attention to the units: here, each computation takes about 490 ns, 490 billionths of a second. To help calibrate the impact of a microbenchmark on run time, it's useful to think about how many times a function needs to run before it takes a second. If a microbenchmark takes:
...
The `sqrt()` function takes about 490 ns, or 0.49 µs, to compute the square root of 100 numbers. That means if you repeated the operation a million times, it would take 0.49 s, and hence changing the way you compute the square root is unlikely to significantly affect real code. This is the reason you need to exercise care when generalising microbenchmarking results.
